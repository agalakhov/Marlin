...
CrealityDWINClass CrealityDWIN;
  ...
  struct Mesh_Settings {
    ...
    void Draw_Bed_Mesh(int16_t selected = -1, uint8_t gridline_width = 1, uint16_t padding_x = 8, uint16_t padding_y_top = 40 + 53 - 7) {
      drawing_mesh = true;
      const uint16_t total_width_px = geometry.screen.w - padding_x - padding_x;
...
void CrealityDWINClass::Clear_Screen(uint8_t e/*=3*/) {
  if (e==1||e==3||e==4) DWIN_Draw_Rectangle(1, GetColor(eeprom_settings.menu_top_bg, Color_Bg_Blue, false), 0, 0, geometry.screen.w, geometry.titleHeight); // Clear Title Bar
  if (e==2||e==3) DWIN_Draw_Rectangle(1, Color_Bg_Black, 0, 31, geometry.screen.w, STATUS_Y); // Clear Menu Area
  if (e==4) DWIN_Draw_Rectangle(1, Color_Bg_Black, 0, 31, geometry.screen.w, geometry.screen.h); // Clear Popup Area
}
...
void CrealityDWINClass::Draw_Float(float value, uint8_t row, bool selected/*=false*/, uint8_t minunit/*=10*/) {
  ...
  if (isnan(value)) {
    DWIN_Draw_String(false, true, font, Color_White, bColor, xpos - 8, MBASE(row), F(" NaN"));
  } else if (value < 0) {
    DWIN_Draw_FloatValue(true, true, 0, font, Color_White, bColor, digits-log10(minunit)+1, log10(minunit), xpos, MBASE(row), -value * minunit);
    DWIN_Draw_String(false, true, font, Color_White, bColor, xpos - 8, MBASE(row), F("-"));
  }
  else {
    DWIN_Draw_FloatValue(true, true, 0, font, Color_White, bColor, digits-log10(minunit)+1, log10(minunit), xpos, MBASE(row), value * minunit);
    DWIN_Draw_String(false, true, font, Color_White, bColor, xpos - 8, MBASE(row), F(" "));
...
void CrealityDWINClass::Draw_Option(uint8_t value, const char * const * options, uint8_t row, bool selected/*=false*/, bool color/*=false*/) {
  ...
  DWIN_Draw_String(false, false, geometry.menuFont, tColor, bColor, 202, MBASE(row) - 1, options[value]);
}
...
void CrealityDWINClass::Draw_String(char * string, uint8_t row, bool selected/*=false*/, bool below/*=false*/) {
  ...
  DWIN_Draw_String(false, true, geometry.menuFont, Color_White, (selected) ? Select_Color : Color_Bg_Black, offset_x, MBASE(row)-1+offset_y, string);
}
...
void CrealityDWINClass::Draw_Menu_Item(uint8_t row, Icon icon/*=0*/, const char * label1, const char * label2, bool more/*=false*/, bool centered/*=false*/) {
  ...
  const uint8_t label2_offset_x = !centered ? LBLX : LBLX * 4/5 + max(LBLX * 1U/5, (geometry.screen.w - LBLX - (label2 ? strlen(label2) : 0) * MENU_CHR_W) / 2);
  if (label1) DWIN_Draw_String(false, false, geometry.menuFont, Color_White, Color_Bg_Black, label1_offset_x, MBASE(row) - 1 - label_offset_y, label1); // Draw Label
  if (label2) DWIN_Draw_String(false, false, geometry.menuFont, Color_White, Color_Bg_Black, label2_offset_x, MBASE(row) - 1 + label_offset_y, label2); // Draw Label
  if (icon != NoIcon) DWIN_ICON_Show(ICON, icon, 26, MBASE(row) - 3);   //Draw Menu Icon
  if (more) DWIN_ICON_Show(ICON, Icon::More, 226, MBASE(row) - 3); // Draw More Arrow
...
void CrealityDWINClass::Draw_Checkbox(uint8_t row, bool value) {
    ...
    DWIN_ICON_Show(ICON, (value ? Icon::Checkbox_T : Icon::Checkbox_F), 226, MBASE(row) - 3); 
...
void CrealityDWINClass::Main_Menu_Icons() {
  ...
  constexpr auto grid = geometry.mainMenuGrid;
  ...
  for (uint8_t row = 0; row < grid.counts.h; ++row) {
    for (uint8_t col = 0; col < grid.counts.w; ++col) {
      const uint16_t idx = row * grid.counts.w + col;
      const bool sel = (selection == idx);
      const auto pos = grid.At(col, row);
      const auto& item = items[idx];
      const auto rect = geometry.mainMenuItemSize.AtPoint(pos);
      DWIN_ICON_Show(ICON, sel ? item.selectedIcon : item.normalIcon, pos.x, pos.y);
      if (sel) {
        DWIN_Draw_Rectangle(0, GetColor(eeprom_settings.highlight_box, Color_White), rect.tl.x, rect.tl.y, rect.br.x, rect.br.y);
      }
      Rectangle textRect = { { rect.tl.x, static_cast<uint16_t>(rect.br.y - geometry.mainMenuTextHeight) }, rect.br };
      auto textPos = geometry.Text_Inscribe(textRect, font, item.text);
...
void CrealityDWINClass::Print_Screen_Icons() {
  ...
  if (selection == 0) {
    ...
    DWIN_ICON_Show(ICON, Icon::Setup_0, 8, 252);
    DWIN_Draw_String(false, false, font, Color_White, Color_Bg_Blue, 30, 322, F("Tune"));
  }
  if (selection == 2) {
    DWIN_ICON_Show(ICON, Icon::Stop_1, 184, 252);
    DWIN_Draw_Rectangle(0, GetColor(eeprom_settings.highlight_box, Color_White), 184, 252, 263, 351);
    DWIN_Draw_String(false, false, font, Color_White, Color_Bg_Blue, 205, 322, F("Stop"));
  }
  else {
    DWIN_ICON_Show(ICON, Icon::Stop_0, 184, 252);
    DWIN_Draw_String(false, false, font, Color_White, Color_Bg_Blue, 205, 322, F("Stop"));
  }
  if (paused) {
    if (selection == 1) {
      DWIN_ICON_Show(ICON, Icon::Continue_1, 96, 252);
      DWIN_Draw_Rectangle(0, GetColor(eeprom_settings.highlight_box, Color_White), 96, 252, 175, 351);
      DWIN_Draw_String(false, false, font, Color_White, Color_Bg_Blue, 114, 322, F("Print"));
    }
    else {
      DWIN_ICON_Show(ICON, Icon::Continue_0, 96, 252);
      DWIN_Draw_String(false, false, font, Color_White, Color_Bg_Blue, 114, 322, F("Print"));
    }
  }
  else {
    if (selection == 1) {
      DWIN_ICON_Show(ICON, Icon::Pause_1, 96, 252);
      DWIN_Draw_Rectangle(0, GetColor(eeprom_settings.highlight_box, Color_White), 96, 252, 175, 351);
      DWIN_Draw_String(false, false, font, Color_White, Color_Bg_Blue, 114, 322, F("Pause"));
    }
    else {
      DWIN_ICON_Show(ICON, Icon::Pause_0, 96, 252);
      DWIN_Draw_String(false, false, font, Color_White, Color_Bg_Blue, 114, 322, F("Pause"));
    }
  }
}
...
void CrealityDWINClass::Draw_Print_Screen() {
  ...
  DWIN_Draw_String(false, false, geometry.menuFont, Color_White, Color_Bg_Black, 41, 163, "Elapsed");
  DWIN_Draw_String(false, false, geometry.menuFont,  Color_White, Color_Bg_Black, 176, 163, "Remaining");
  Update_Status_Bar(true);
...
void CrealityDWINClass::Draw_Print_Filename(bool reset/*=false*/) {
  ...
  if (process == Print) {
    ...
    if (pos > 30) {
      ...
      dispname[len] = '\0';
      DWIN_Draw_Rectangle(1, Color_Bg_Black, 8, 50, geometry.screen.w-8, 80);
      const int8_t npos = (geometry.screen.w - 30 * MENU_CHR_W) / 2;
      DWIN_Draw_String(false, false, geometry.menuFont, Color_White, Color_Bg_Black, npos, 60, dispname);
      if (-pos >= 30)
        namescrl = 0;
      namescrl++;
    }
      ...
      DWIN_Draw_Rectangle(1, Color_Bg_Black, 8, 50, geometry.screen.w-8, 80);
      const int8_t npos = (geometry.screen.w - strlen(filename) * MENU_CHR_W) / 2;
      DWIN_Draw_String(false, false, geometry.menuFont, Color_White, Color_Bg_Black, npos, 60, filename);
    }
  }
}
...
void CrealityDWINClass::Draw_Print_confirm() {
  ...
  DWIN_ICON_Show(ICON, Icon::Confirm_E, 87, 283);
...
void CrealityDWINClass::Draw_SD_Item(uint8_t item, uint8_t row) {
  if (item == 0) {
    if (card.flag.workDirIsRoot)
      Draw_Menu_Item(0, Icon::Back, "Back");
    else
      Draw_Menu_Item(0, Icon::Back, "..");
  }
  else {
    ...
    name[len] = '\0';
    Draw_Menu_Item(row, card.flag.filenameIsDir ? Icon::More : Icon::File, name);
  }
}
...
void CrealityDWINClass::Draw_SD_List(bool removed/*=false*/) {
  ...
  if (card.isMounted() && !removed) {
    ...
    Draw_Menu_Item(0, Icon::Back, "Back");
    DWIN_Draw_Rectangle(1, Color_Bg_Red, 10, MBASE(3) - 10, geometry.screen.w - 10, MBASE(4));
    DWIN_Draw_String(false, false, Font::font16x32, Color_Yellow, Color_Bg_Red, ((geometry.screen.w) - 8 * 16) / 2, MBASE(3), "No Media");
  }
...
void CrealityDWINClass::Draw_Status_Area(bool icons/*=false*/) {
  ...
  if(icons) DWIN_Draw_Rectangle(1, Color_Bg_Black, 0, STATUS_Y, geometry.screen.w, geometry.screen.h - 1);
    ...
    if (icons) { 
      hotend = -1;
      hotendtarget = -1;
      DWIN_ICON_Show(ICON, Icon::HotendTemp, 10, 383);
      DWIN_Draw_String(false, false, geometry.statusFont, GetColor(eeprom_settings.status_area_text, Color_White), Color_Bg_Black, 25 + 3 * STAT_CHR_W + 5, 384, F("/"));
    }
    if (thermalManager.temp_hotend[0].celsius != hotend) {
      hotend = thermalManager.temp_hotend[0].celsius;
      DWIN_Draw_IntValue(true, true, 0, geometry.statusFont, GetColor(eeprom_settings.status_area_text, Color_White), Color_Bg_Black, 3, 28, 384, thermalManager.temp_hotend[0].celsius);
    ...
    if (thermalManager.temp_hotend[0].target != hotendtarget) {
      ...
      DWIN_Draw_IntValue(true, true, 0, geometry.statusFont, GetColor(eeprom_settings.status_area_text, Color_White), Color_Bg_Black, 3, 25 + 4 * STAT_CHR_W + 6, 384, thermalManager.temp_hotend[0].target);
      DWIN_Draw_DegreeSymbol(GetColor(eeprom_settings.status_area_text, Color_White), 25 + 4 * STAT_CHR_W + 39, 386);
    }
    if (icons) {
      flow = -1;
      DWIN_ICON_Show(ICON, Icon::StepE, 112, 417);
      DWIN_Draw_String(false, false, geometry.statusFont, GetColor(eeprom_settings.status_area_text, Color_White), Color_Bg_Black, 116 + 5 * STAT_CHR_W + 2, 417, F("%"));
    } 
    if (planner.flow_percentage[0] != flow) {
      flow = planner.flow_percentage[0];
      DWIN_Draw_IntValue(true, true, 0, geometry.statusFont, GetColor(eeprom_settings.status_area_text, Color_White), Color_Bg_Black, 3, 116 + 2 * STAT_CHR_W, 417, planner.flow_percentage[0]);
    }
  #endif
    ...
    if (icons) {
      bed = -1;
      bedtarget = -1;
      DWIN_ICON_Show(ICON, Icon::BedTemp, 10, 416);
      DWIN_Draw_String(false, false, geometry.statusFont, GetColor(eeprom_settings.status_area_text, Color_White), Color_Bg_Black, 25 + 3 * STAT_CHR_W + 5, 417, F("/"));
    }
    if (thermalManager.temp_bed.celsius != bed) {
      bed = thermalManager.temp_bed.celsius;
      DWIN_Draw_IntValue(true, true, 0, geometry.statusFont, GetColor(eeprom_settings.status_area_text, Color_White), Color_Bg_Black, 3, 28, 417, thermalManager.temp_bed.celsius);
    ...
    if (thermalManager.temp_bed.target != bedtarget) {
      ...
      DWIN_Draw_IntValue(true, true, 0, geometry.statusFont, GetColor(eeprom_settings.status_area_text, Color_White), Color_Bg_Black, 3, 25 + 4 * STAT_CHR_W + 6, 417, thermalManager.temp_bed.target);
    ...
    if (icons) {
      fan = -1;
      DWIN_ICON_Show(ICON, Icon::FanSpeed, 187, 383);
    }
    if (thermalManager.fan_speed[0] != fan) {
      fan = thermalManager.fan_speed[0];
      DWIN_Draw_IntValue(true, true, 0, geometry.statusFont, GetColor(eeprom_settings.status_area_text, Color_White), Color_Bg_Black, 3, 195 + 2 * STAT_CHR_W, 384, thermalManager.fan_speed[0]);
    }
  #endif
    ...
    if (icons) {
      offset = -1;
      DWIN_ICON_Show(ICON, Icon::Zoffset, 187, 416);
    }
    if (zoffsetvalue != offset) {
      offset = zoffsetvalue;
      if (zoffsetvalue < 0) {
        DWIN_Draw_FloatValue(true, true, 0, geometry.statusFont, GetColor(eeprom_settings.status_area_text, Color_White), Color_Bg_Black, 2, 2, 207, 417, -zoffsetvalue * 100);
        DWIN_Draw_String(false, true, geometry.menuFont, GetColor(eeprom_settings.status_area_text, Color_White), Color_Bg_Black, 205, 419, "-");
      }
      else {
        DWIN_Draw_FloatValue(true, true, 0, geometry.statusFont, GetColor(eeprom_settings.status_area_text, Color_White), Color_Bg_Black, 2, 2, 207, 417, zoffsetvalue* 100);
        DWIN_Draw_String(false, true, geometry.menuFont, GetColor(eeprom_settings.status_area_text, Color_White), Color_Bg_Black, 205, 419, " ");
  ...
  if (icons) {
    feedrate = -1;
    DWIN_ICON_Show(ICON, Icon::Speed, 113, 383);
    DWIN_Draw_String(false, false, geometry.statusFont, GetColor(eeprom_settings.status_area_text, Color_White), Color_Bg_Black, 116 + 5 * STAT_CHR_W + 2, 384, F("%"));
  }
  if (feedrate_percentage != feedrate) {
    feedrate = feedrate_percentage;
    DWIN_Draw_IntValue(true, true, 0, geometry.statusFont, GetColor(eeprom_settings.status_area_text, Color_White), Color_Bg_Black, 3, 116 + 2 * STAT_CHR_W, 384, feedrate_percentage);
  }
  ...
  if (icons) {
    ...
    DWIN_ICON_Show(ICON, Icon::MaxSpeedX,   10, 456);
    DWIN_ICON_Show(ICON, Icon::MaxSpeedY,   95, 456);
    DWIN_ICON_Show(ICON, Icon::MaxSpeedZ,   180, 456);
  }
  if (update_x) {
    x = current_position.x;
    if ((update_x = axis_should_home(X_AXIS) && ui.get_blink()))
      DWIN_Draw_String(false, true, geometry.menuFont, GetColor(eeprom_settings.coordinates_text, Color_White), Color_Bg_Black, 35, 459, "  -?-  ");
    else
      DWIN_Draw_FloatValue(true, true, 0, geometry.menuFont, GetColor(eeprom_settings.coordinates_text, Color_White), Color_Bg_Black, 3, 1, 35, 459, current_position.x * 10);
  }
  if (update_y) {
    y = current_position.y;
    if ((update_y = axis_should_home(Y_AXIS) && ui.get_blink()))
      DWIN_Draw_String(false, true, geometry.menuFont, GetColor(eeprom_settings.coordinates_text, Color_White), Color_Bg_Black, 120, 459, "  -?-  ");
    else
      DWIN_Draw_FloatValue(true, true, 0, geometry.menuFont, GetColor(eeprom_settings.coordinates_text, Color_White), Color_Bg_Black, 3, 1, 120, 459, current_position.y * 10);
  }
  if (update_z) {
    z = current_position.z;
    if ((update_z = axis_should_home(Z_AXIS) && ui.get_blink()))
      DWIN_Draw_String(false, true, geometry.menuFont, GetColor(eeprom_settings.coordinates_text, Color_White), Color_Bg_Black, 205, 459, "  -?-  ");
        else
      DWIN_Draw_FloatValue(true, true, 0, geometry.menuFont, GetColor(eeprom_settings.coordinates_text, Color_White), Color_Bg_Black, 3, 2, 205, 459, (current_position.z>=0) ? current_position.z * 100 : 0);
  }
  DWIN_UpdateLCD();
}

void CrealityDWINClass::Draw_Popup(const char *line1, const char *line2,const char *line3, uint8_t mode, Icon icon/*=NoIcon*/) {
  ...
  DWIN_Draw_String(false, true, geometry.menuFont, Popup_Text_Color, Color_Bg_Window, (272 - 8 * strlen(line1)) / 2, ypos, F(line1));
  DWIN_Draw_String(false, true, geometry.menuFont, Popup_Text_Color, Color_Bg_Window, (272 - 8 * strlen(line2)) / 2, ypos+30, F(line2));
  DWIN_Draw_String(false, true, geometry.menuFont, Popup_Text_Color, Color_Bg_Window, (272 - 8 * strlen(line3)) / 2, ypos+60, F(line3));
  if (mode == Popup) {
    selection = 0;
    DWIN_Draw_Rectangle(1, Confirm_Color, 26, 280, 125, 317);
    DWIN_Draw_Rectangle(1, Cancel_Color, 146, 280, 245, 317);
    DWIN_Draw_String(false, false, geometry.statusFont, Color_White, Color_Bg_Window, 39, 290, "Confirm");
    DWIN_Draw_String(false, false, geometry.statusFont, Color_White, Color_Bg_Window, 165, 290, "Cancel");
    Popup_Select();
  }
  else if (mode == Confirm) {
    DWIN_Draw_Rectangle(1, Confirm_Color, 87, 280, 186, 317);
    DWIN_Draw_String(false, false, geometry.statusFont, Color_White, Color_Bg_Window, 96, 290, "Continue");
  }
}
...
void CrealityDWINClass::Update_Status_Bar(bool refresh/*=false*/) {
  ...
  if (pos > 30) {
    ...
    dispmsg[len] = '\0';
    if (process == Print) {
      DWIN_Draw_Rectangle(1, Color_Grey, 8, 214, geometry.screen.w-8, 238);
      const int8_t npos = (geometry.screen.w - 30 * MENU_CHR_W) / 2;
      DWIN_Draw_String(false, false, geometry.menuFont, GetColor(eeprom_settings.status_bar_text, Color_White), Color_Bg_Black, npos, 219, dispmsg);
    }
    else {
      DWIN_Draw_Rectangle(1, Color_Bg_Black, 8, 352, geometry.screen.w-8, 376);
      const int8_t npos = (geometry.screen.w - 30 * MENU_CHR_W) / 2;
      DWIN_Draw_String(false, false, geometry.menuFont, GetColor(eeprom_settings.status_bar_text, Color_White), Color_Bg_Black, npos, 357, dispmsg);
    }
    if (-pos >= 30)
      msgscrl = 0;
    msgscrl++;
    ...
    if (new_msg) {
      ...
      if (process == Print) {
        DWIN_Draw_Rectangle(1, Color_Grey, 8, 214, geometry.screen.w-8, 238);
        const int8_t npos = (geometry.screen.w - strlen(statusmsg) * MENU_CHR_W) / 2;
        DWIN_Draw_String(false, false, geometry.menuFont, GetColor(eeprom_settings.status_bar_text, Color_White), Color_Bg_Black, npos, 219, statusmsg);
      }
      else {
        DWIN_Draw_Rectangle(1, Color_Bg_Black, 8, 352, geometry.screen.w-8, 376);
        const int8_t npos = (geometry.screen.w - strlen(statusmsg) * MENU_CHR_W) / 2;
        DWIN_Draw_String(false, false, geometry.menuFont, GetColor(eeprom_settings.status_bar_text, Color_White), Color_Bg_Black, npos, 357, statusmsg);
      }
    }
  }
}
...
void CrealityDWINClass::Draw_Keys(uint8_t index, bool selected, bool uppercase/*=false*/, bool lock/*=false*/) {
  ...
  if (index < 28) {
    if (index == 19) {
      ...
      DWIN_Draw_String(false, true, geometry.menuFont, Color_Red, Color_Bg_Black, KEY_X1(7)+xOffset[2]+3, KEY_Y1(2)+5, "<--");
    }
    else {
      if (index > 19) index--;
      if (index > 27) index--;
      ...
      DWIN_Draw_String(false, false, geometry.menuFont, Color_White, Color_Bg_Black, KEY_X1(x)+xOffset[y]+5, KEY_Y1(y)+5, keyStr);
    ...
    switch (index) {
      case 28:
        ...
        DWIN_Draw_String(false, false, geometry.menuFont, Color_White, Color_Bg_Black, KEY_X1(0)-1, KEY_Y1(3)+5, "?123");
        break;
      case 29:
        ...
        DWIN_Draw_String(false, false, geometry.menuFont, Color_White, Color_Bg_Black, KEY_X1(1)+xOffset[1]+5, KEY_Y1(3)+5, "-");
        break;
      case 30:
        ...
        DWIN_Draw_String(false, false, geometry.menuFont, Color_White, Color_Bg_Black, KEY_X1(2)+xOffset[1]+5, KEY_Y1(3)+5, "_");
        break;
      case 31:
        ...
        DWIN_Draw_String(false, false, geometry.menuFont, Color_White, Color_Bg_Black, KEY_X1(3)+xOffset[1]+14, KEY_Y1(3)+5, "Space");
      ...
      case 32:
        ...
        DWIN_Draw_String(false, false, geometry.menuFont, Color_White, Color_Bg_Black, KEY_X1(6)+xOffset[1]+7, KEY_Y1(3)+5, ".");
        if (keyboard_restrict) {
      ...
      case 33:
        ...
        DWIN_Draw_String(false, false, geometry.menuFont, Color_White, Color_Bg_Black, KEY_X1(7)+xOffset[1]+4, KEY_Y1(3)+5, "/");
        if (keyboard_restrict) {
      ...
      case 34:
        ...
        DWIN_Draw_String(false, true, geometry.menuFont, Color_Cyan, Color_Bg_Black, KEY_X1(7)+xOffset[2]+3, KEY_Y1(3)+5, "-->");
        break;
    }
  }
}
  ...
  switch (menu) {
    case Prepare:
      ...
      switch (item) {
        case PREPARE_BACK:
          if (draw) {
            Draw_Menu_Item(row, Icon::Back, "Back");
          }
          else {
            Draw_Main_Menu(1);
          }
        ...
        case PREPARE_MOVE:
          if (draw) {
            Draw_Menu_Item(row, Icon::Axis, "Move", NULL, true);
          }
          else {
            Draw_Menu(Move);
          }
        ...
        case PREPARE_DISABLE:
          if (draw) {
            Draw_Menu_Item(row, Icon::CloseMotor, "Disable Stepper");
          }
          else {
        ...
        case PREPARE_HOME:
          if (draw) {
            Draw_Menu_Item(row, Icon::SetHome, "Homing", NULL, true);
          }
          else {
            Draw_Menu(HomeMenu);
          }
        ...
        case PREPARE_MANUALLEVEL:
          if (draw) {
            Draw_Menu_Item(row, Icon::PrintSize, "Manual Leveling", NULL, true);
          }
          else {
            if (axes_should_home()) {
              Popup_Handler(Home);
          ...
          case PREPARE_ZOFFSET:
            if (draw) {
              Draw_Menu_Item(row, Icon::Zoffset, "Z-Offset", NULL, true);
            }
            else {
              #if HAS_LEVELING
                level_state = planner.leveling_active;
          ...
          case PREPARE_PREHEAT:
            if (draw) {
              Draw_Menu_Item(row, Icon::Temperature, "Preheat", NULL, true);
            }
            else {
              Draw_Menu(Preheat);
            }
          ...
          case PREPARE_COOLDOWN:
            if (draw) {
              Draw_Menu_Item(row, Icon::Cool, "Cooldown");
            } 
            else {
          ...
          case PREPARE_CHANGEFIL:
            if (draw) {
                ...
                Draw_Menu_Item(row, Icon::ResumeEEPROM, "Change Filament", NULL, true);
              #else
                Draw_Menu_Item(row, Icon::ResumeEEPROM, "Change Filament");
              #endif
            }
            else {
              #if ENABLED(FILAMENT_LOAD_UNLOAD_GCODES)
        ...
        case PREPARE_ACTIONCOMMANDS:
          if (draw) {
            Draw_Menu_Item(row, Icon::SetHome, "Host Actions", NULL, true);
          }
          else {
    ...
    case HomeMenu:
      ...
      switch(item) {
        ...
        case HOME_ALL:
          if (draw) {
            Draw_Menu_Item(row, Icon::Homing, "Home All");
          }
          else {
            Popup_Handler(Home);
            gcode.home_all_axes(true);
        ...
        case HOME_X:
          if (draw) {
            Draw_Menu_Item(row, Icon::MoveX, "Home X");
          }
          else {
            Popup_Handler(Home);
        ...
        case HOME_Y:
          if (draw) {
            Draw_Menu_Item(row, Icon::MoveY, "Home Y");
          }
          else {
            Popup_Handler(Home);
        ...
        case HOME_Z:
          if (draw) {
            Draw_Menu_Item(row, Icon::MoveZ,"Home Z");
          }
          else {
            Popup_Handler(Home);
        ...
        case HOME_SET:
          if (draw) {
            Draw_Menu_Item(row, Icon::SetHome, "Set Home Position");
          }
          else {
    ...
    case Move:
      ...
      switch (item) {
        ...
        case MOVE_X:
          if (draw) {
            Draw_Menu_Item(row, Icon::MoveX, "Move X");
        ...
        case MOVE_Y:
          if (draw) {
            Draw_Menu_Item(row, Icon::MoveY, "Move Y");
        ...
        case MOVE_Z:
          if (draw) {
            Draw_Menu_Item(row, Icon::MoveZ, "Move Z");
          ...
          case MOVE_E:
            if (draw) {
              Draw_Menu_Item(row, Icon::Extruder, "Extruder");
          ...
          case MOVE_P:
            if (draw) {
              Draw_Menu_Item(row, Icon::StockConfiguraton, "Probe");
        ...
        case MOVE_LIVE:
          if (draw) {
            Draw_Menu_Item(row, Icon::Axis, "Live Movement");
    ...
    case ManualLevel:
      ...
      switch (item) {
        case MLEVEL_BACK:
          ...
          case MLEVEL_PROBE:
            if (draw) {
              Draw_Menu_Item(row, Icon::Zoffset, "Use Probe");
        ...
        case MLEVEL_BL:
          if (draw) {
            Draw_Menu_Item(row, Icon::AxisBL, "Bottom Left");
          }
          else {
        ...
        case MLEVEL_TL:
          if (draw) {
            Draw_Menu_Item(row, Icon::AxisTL, "Top Left");
          }
          else {
            Popup_Handler(MoveWait);
            if (use_probe) {
        ...
        case MLEVEL_TR:
          if (draw) {
            Draw_Menu_Item(row, Icon::AxisTR, "Top Right");
          }
          else {
            Popup_Handler(MoveWait);
            if (use_probe) {
        ...
        case MLEVEL_BR:
          if (draw) {
            Draw_Menu_Item(row, Icon::AxisBR, "Bottom Right");
          }
          else {
            Popup_Handler(MoveWait);
            if (use_probe) {
        ...
        case MLEVEL_C:
          if (draw) {
            Draw_Menu_Item(row, Icon::AxisC, "Center");
          }
          else {
            Popup_Handler(MoveWait);
            if (use_probe) {
        ...
        case MLEVEL_ZPOS:
          if (draw) {
            Draw_Menu_Item(row, Icon::SetZOffset, "Z Position");
      ...
      case ZOffset:
        ...
        switch (item) {
          ...
          case ZOFFSET_HOME:
            if (draw) {
              Draw_Menu_Item(row, Icon::Homing, "Home Z Axis");
            }
            else {
              Popup_Handler(Home);
              gcode.process_subcommands_now_P(PSTR("G28 Z"));
          ...
          case ZOFFSET_MODE:
            if (draw) {
              Draw_Menu_Item(row, Icon::Zoffset, "Live Adjustment");
          ...
          case ZOFFSET_OFFSET:
            if (draw) {
              Draw_Menu_Item(row, Icon::SetZOffset, "Z Offset");
          ...
          case ZOFFSET_UP:
            if (draw) {
              Draw_Menu_Item(row, Icon::Axis, "Microstep Up");
            }
            else {
          ...
          case ZOFFSET_DOWN:
            if (draw) {
              Draw_Menu_Item(row, Icon::AxisD, "Microstep Down");
            }
            else {
            ...
            case ZOFFSET_SAVE:
              if (draw) {
                Draw_Menu_Item(row, Icon::WriteEEPROM, "Save");
              }
              else {
      ...
      case Preheat:
        ...
        switch (item) {
          ...
          case PREHEAT_MODE:
            if (draw) {
              Draw_Menu_Item(row, Icon::Homing, "Preheat Mode");
            ...
            case PREHEAT_1:
              if (draw) {
                Draw_Menu_Item(row, Icon::Temperature, PREHEAT_1_LABEL);
              }
              else {
                thermalManager.disable_all_heaters();
                thermalManager.zero_fan_speeds();
            ...
            case PREHEAT_2:
              if (draw) {
                Draw_Menu_Item(row, Icon::Temperature, PREHEAT_2_LABEL);
              }
              else {
                thermalManager.disable_all_heaters();
                thermalManager.zero_fan_speeds();
            ...
            case PREHEAT_3:
              if (draw) {
                Draw_Menu_Item(row, Icon::Temperature, PREHEAT_3_LABEL);
              }
              else {
                thermalManager.disable_all_heaters();
                thermalManager.zero_fan_speeds();
            ...
            case PREHEAT_4:
              if (draw) {
                Draw_Menu_Item(row, Icon::Temperature, PREHEAT_4_LABEL);
              }
              else {
                thermalManager.disable_all_heaters();
                thermalManager.zero_fan_speeds();
            ...
            case PREHEAT_5:
              if (draw) {
                Draw_Menu_Item(row, Icon::Temperature, PREHEAT_5_LABEL);
              }
              else {
                thermalManager.disable_all_heaters();
                thermalManager.zero_fan_speeds();
      ...
      case ChangeFilament:
        ...
        switch (item) {
          ...
          case CHANGEFIL_LOAD:
            if (draw) {
              Draw_Menu_Item(row, Icon::WriteEEPROM, "Load Filament");
            }
            else {
              if (thermalManager.temp_hotend[0].target < thermalManager.extrude_min_temp) {
                Popup_Handler(ETemp);
          ...
          case CHANGEFIL_UNLOAD:
            if (draw) {
              Draw_Menu_Item(row, Icon::ReadEEPROM, "Unload Filament");
            }
            else {
              if (thermalManager.temp_hotend[0].target < thermalManager.extrude_min_temp) {
                Popup_Handler(ETemp);
    ...
    case HostActions:
      ...
      switch(item) {
        ...
        case HOSTACTIONS_1:
          if (draw) {
            Draw_Menu_Item(row, Icon::File, action1);
          }
          else {
        ...
        case HOSTACTIONS_2:
          if (draw) {
            Draw_Menu_Item(row, Icon::File, action2);
          }
          else {
        ...
        case HOSTACTIONS_3:
          if (draw) {
            Draw_Menu_Item(row, Icon::File, action3);
          }
          else {
        ...
        case HOSTACTIONS_4:
          if (draw) {
            Draw_Menu_Item(row, Icon::File, action4);
          }
          else {
        ...
        case HOSTACTIONS_5:
          if (draw) {
            Draw_Menu_Item(row, Icon::File, action5);
          }
          else {
    ...
    case Control:
      ...
      switch (item) {
        ...
        case CONTROL_TEMP:
          if (draw) {
            Draw_Menu_Item(row, Icon::Temperature, "Temperature", NULL, true);
          }
          else {
            Draw_Menu(TempMenu);
          }
        ...
        case CONTROL_MOTION:
          if (draw) {
            Draw_Menu_Item(row, Icon::Motion, "Motion", NULL, true);
          }
          else {
            Draw_Menu(Motion);
          }
        ...
        case CONTROL_VISUAL:
          if (draw) {
            Draw_Menu_Item(row, Icon::PrintSize, "Visual", NULL, true);
          }
          else {
            Draw_Menu(Visual);
          }
        ...
        case CONTROL_HOSTSETTINGS:
          if (draw) {
            Draw_Menu_Item(row, Icon::Contact, "Host Settings", NULL, true);
          }
          else {
        ...
        case CONTROL_ADVANCED:
          if (draw) {
            Draw_Menu_Item(row, Icon::Version, "Advanced", NULL, true);
          }
          else {
            Draw_Menu(Advanced);
          }
          ...
          case CONTROL_SAVE:
            if (draw) {
              Draw_Menu_Item(row, Icon::WriteEEPROM, "Store Settings");
            }
            else {
              AudioFeedback(settings.save());
            }
          ...
          case CONTROL_RESTORE:
            if (draw) {
              Draw_Menu_Item(row, Icon::ReadEEPROM, "Restore Settings");
            }
            else {
          ...
          case CONTROL_RESET:
            if (draw) {
              Draw_Menu_Item(row, Icon::Temperature, "Reset to Defaults");
            }
            else {
              settings.reset();
              AudioFeedback();
        ...
        case CONTROL_INFO:
          if (draw) {
            Draw_Menu_Item(row, Icon::Info, "Info");
          }
          else {
            Draw_Menu(Info);
          }
    ...
    case TempMenu:
      ...
      switch (item) {
        case TEMP_BACK:
          ...
          case TEMP_HOTEND:
            if (draw) {
              Draw_Menu_Item(row, Icon::SetEndTemp, "Hotend");
          ...
          case TEMP_BED:
            if (draw) {
              Draw_Menu_Item(row, Icon::SetBedTemp, "Bed");
          ...
          case TEMP_FAN:
            if (draw) {
              Draw_Menu_Item(row, Icon::FanSpeed, "Fan");
          ...
          case TEMP_PID:
            if (draw) {
              Draw_Menu_Item(row, Icon::Step, "PID", NULL, true);
            }
            else {
              Draw_Menu(PID);
            }
          ...
          case TEMP_PREHEAT1:
            if (draw) {
              Draw_Menu_Item(row, Icon::Step, PREHEAT_1_LABEL, NULL, true);
            }
            else {
              Draw_Menu(Preheat1);
            }
          ...
          case TEMP_PREHEAT2:
            if (draw) {
              Draw_Menu_Item(row, Icon::Step, PREHEAT_2_LABEL, NULL, true);
            }
            else {
              Draw_Menu(Preheat2);
            }
          ...
          case TEMP_PREHEAT3:
            if (draw) {
              Draw_Menu_Item(row, Icon::Step, PREHEAT_3_LABEL, NULL, true);
            }
            else {
              Draw_Menu(Preheat3);
            }
          ...
          case TEMP_PREHEAT4:
            if (draw) {
              Draw_Menu_Item(row, Icon::Step, PREHEAT_4_LABEL, NULL, true);
            }
            else {
              Draw_Menu(Preheat4);
            }
          ...
          case TEMP_PREHEAT5:
            if (draw) {
              Draw_Menu_Item(row, Icon::Step, PREHEAT_5_LABEL, NULL, true);
            }
            else {
              Draw_Menu(Preheat5);
            }
      ...
      case PID:
        ...
        switch (item) {
          case PID_BACK:
            ...
            case PID_HOTEND:
              if (draw) {
                Draw_Menu_Item(row, Icon::HotendTemp, "Hotend", NULL, true);
              }
              else {
                Draw_Menu(HotendPID);
              }
            ...
            case PID_BED:
              if (draw) {
                Draw_Menu_Item(row, Icon::BedTemp, "Bed", NULL, true);
              }
              else {
                Draw_Menu(BedPID);
              }
          ...
          case PID_CYCLES:
            if (draw) {
              Draw_Menu_Item(row, Icon::FanSpeed, "Cycles");
      ...
      case HotendPID:
        ...
        switch (item) {
          ...
          case HOTENDPID_TUNE:
            if (draw) {
              Draw_Menu_Item(row, Icon::HotendTemp, "Autotune");
            }
            else {
          ...
          case HOTENDPID_TEMP:
            if (draw) {
              Draw_Menu_Item(row, Icon::Temperature, "Temperature");
          ...
          case HOTENDPID_KP:
            if (draw) {
              Draw_Menu_Item(row, Icon::Version, "Kp Value");
          ...
          case HOTENDPID_KI:
            if (draw) {
              Draw_Menu_Item(row, Icon::Version, "Ki Value");
          ...
          case HOTENDPID_KD:
            if (draw) {
              Draw_Menu_Item(row, Icon::Version, "Kd Value");
    ...
    case Motion:
      ...
      switch (item) {
        ...
        case MOTION_HOMEOFFSETS:
          if (draw) {
            Draw_Menu_Item(row, Icon::SetHome, "Home Offsets", NULL, true);
          }
          else {
        ...
        case MOTION_SPEED:
          if (draw) {
            Draw_Menu_Item(row, Icon::MaxSpeed, "Max Speed", NULL, true);
          }
          else {
            Draw_Menu(MaxSpeed);
          }
        ...
        case MOTION_ACCEL:
          if (draw) {
            Draw_Menu_Item(row, Icon::MaxAccelerated, "Max Acceleration", NULL, true);
          }
          else {
          ...
          case MOTION_JERK:
            if (draw) {
              Draw_Menu_Item(row, Icon::MaxJerk, "Max Jerk", NULL, true);
            }
            else {
              Draw_Menu(MaxJerk);
            }
        ...
        case MOTION_STEPS:
          if (draw) {
            Draw_Menu_Item(row, Icon::Step, "Steps/mm", NULL, true);
          }
          else {
            Draw_Menu(Steps);
          }
          ...
          case MOTION_FLOW:
            if (draw) {
              Draw_Menu_Item(row, Icon::Speed, "Flow Rate");
    ...
    case HomeOffsets:
      ...
      switch (item) {
        ...
        case HOMEOFFSETS_XOFFSET:
          if (draw) {
            Draw_Menu_Item(row, Icon::StepX, "X Offset");
        ...
        case HOMEOFFSETS_YOFFSET:
          if (draw) {
            Draw_Menu_Item(row, Icon::StepY, "Y Offset");
    ...
    case MaxSpeed:
      ...
      switch (item) {
        ...
        case SPEED_X:
          if (draw) {
            Draw_Menu_Item(row, Icon::MaxSpeedX, "X Axis");
        ...
        case SPEED_Y:
          if (draw) {
            Draw_Menu_Item(row, Icon::MaxSpeedY, "Y Axis");
        ...
        case SPEED_Z:
          if (draw) {
            Draw_Menu_Item(row, Icon::MaxSpeedZ, "Z Axis");
          ...
          case SPEED_E:
            if (draw) {
              Draw_Menu_Item(row, Icon::MaxSpeedE, "Extruder");
    ...
    case MaxAcceleration:
      ...
      switch (item) {
        ...
        case ACCEL_X:
          if (draw) {
            Draw_Menu_Item(row, Icon::MaxAccX, "X Axis");
        ...
        case ACCEL_Y:
          if (draw) {
            Draw_Menu_Item(row, Icon::MaxAccY, "Y Axis");
        ...
        case ACCEL_Z:
          if (draw) {
            Draw_Menu_Item(row, Icon::MaxAccZ, "Z Axis");
          ...
          case ACCEL_E:
            if (draw) {
              Draw_Menu_Item(row, Icon::MaxAccE, "Extruder");
      ...
      case MaxJerk:
        ...
        switch (item) {
          ...
          case JERK_X:
            if (draw) {
              Draw_Menu_Item(row, Icon::MaxSpeedJerkX, "X Axis");
          ...
          case JERK_Y:
            if (draw) {
              Draw_Menu_Item(row, Icon::MaxSpeedJerkY, "Y Axis");
          ...
          case JERK_Z:
            if (draw) {
              Draw_Menu_Item(row, Icon::MaxSpeedJerkZ, "Z Axis");
            ...
            case JERK_E:
              if (draw) {
                Draw_Menu_Item(row, Icon::MaxSpeedJerkE, "Extruder");
    ...
    case Steps:
      ...
      switch (item) {
        ...
        case STEPS_X:
          if (draw) {
            Draw_Menu_Item(row, Icon::StepX, "X Axis");
        ...
        case STEPS_Y:
          if (draw) {
            Draw_Menu_Item(row, Icon::StepY, "Y Axis");
        ...
        case STEPS_Z:
          if (draw) {
            Draw_Menu_Item(row, Icon::StepZ, "Z Axis");
          ...
          case STEPS_E:
            if (draw) {
              Draw_Menu_Item(row, Icon::StepE, "Extruder");
    ...
    case Visual:
      ...
      switch (item) {
        ...
        case VISUAL_BACKLIGHT:
          if (draw) {
            Draw_Menu_Item(row, Icon::Brightness, "Display Off");
          }
          else {
            ui.set_brightness(0);
          }
        ...
        case VISUAL_BRIGHTNESS:
          if (draw) {
            Draw_Menu_Item(row, Icon::Brightness, "LCD Brightness");
        ...
        case VISUAL_TIME_FORMAT:
          if (draw) {
            Draw_Menu_Item(row, Icon::PrintTime, "Progress as __h__m");
        ...
        if (draw) {
            Draw_Menu_Item(row, Icon::MaxSpeed, "UI Color Settings", NULL, true);
          }
          else {
    ...
    case ColorSettings:
        ...
        case COLORSETTINGS_CURSOR:
           if (draw) {
            Draw_Menu_Item(row, Icon::MaxSpeed, "Cursor");
          ...
          case COLORSETTINGS_SPLIT_LINE:
           if (draw) {
            Draw_Menu_Item(row, Icon::MaxSpeed, "Menu Split Line");
        ...
        case COLORSETTINGS_MENU_TOP_TXT:
           if (draw) {
            Draw_Menu_Item(row, Icon::MaxSpeed, "Menu Header Text");
         ...
         case COLORSETTINGS_MENU_TOP_BG:
           if (draw) {
            Draw_Menu_Item(row, Icon::MaxSpeed, "Menu Header Bg");
          ...
          case COLORSETTINGS_HIGHLIGHT_BORDER:
           if (draw) {
            Draw_Menu_Item(row, Icon::MaxSpeed, "Highlight Box");
          ...
          case COLORSETTINGS_PROGRESS_PERCENT:
           if (draw) {
            Draw_Menu_Item(row, Icon::MaxSpeed, "Progress Percent");
          ...
          case COLORSETTINGS_PROGRESS_TIME:
           if (draw) {
            Draw_Menu_Item(row, Icon::MaxSpeed, "Progress Time");
          ...
          case COLORSETTINGS_PROGRESS_STATUS_BAR:
           if (draw) {
            Draw_Menu_Item(row, Icon::MaxSpeed, "Status Bar Text");
          ...
          case COLORSETTINGS_PROGRESS_STATUS_AREA:
           if (draw) {
            Draw_Menu_Item(row, Icon::MaxSpeed, "Status Area Text");
          ...
          case COLORSETTINGS_PROGRESS_COORDINATES:
           if (draw) {
            Draw_Menu_Item(row, Icon::MaxSpeed, "Coordinates Text");
          ...
          case COLORSETTINGS_PROGRESS_COORDINATES_LINE:
           if (draw) {
            Draw_Menu_Item(row, Icon::MaxSpeed, "Coordinates Line");
    ...
    case HostSettings:
      ...
      switch (item) {
        ...
        case HOSTSETTINGS_ACTIONCOMMANDS:
          if (draw) {
            Draw_Menu_Item(row, Icon::File, "Action Commands");
          }
          else {
    ...
    case ActionCommands:
      ...
      switch (item) {
        ...
        case ACTIONCOMMANDS_1:
          if (draw) {
            Draw_Menu_Item(row, Icon::File, "Action #1");
        ...
        case ACTIONCOMMANDS_2:
          if (draw) {
            Draw_Menu_Item(row, Icon::File, "Action #2");
        ...
        case ACTIONCOMMANDS_3:
          if (draw) {
            Draw_Menu_Item(row, Icon::File, "Action #3");
        ...
        case ACTIONCOMMANDS_4:
          if (draw) {
            Draw_Menu_Item(row, Icon::File, "Action #4");
        ...
        case ACTIONCOMMANDS_5:
          if (draw) {
            Draw_Menu_Item(row, Icon::File, "Action #5");
    ...
    case Advanced:
      ...
      switch (item) {
        ...
        case ADVANCED_BEEPER:
          if (draw) {
            Draw_Menu_Item(row, Icon::Version, "LCD Beeper");
          ...
          case ADVANCED_PROBE:
            if (draw) {
              Draw_Menu_Item(row, Icon::StepX, "Probe", NULL, true);
            }
            else {
              Draw_Menu(ProbeMenu);
            }
        ...
        case ADVANCED_CORNER:
          if (draw) {
            Draw_Menu_Item(row, Icon::MaxAccelerated, "Bed Screw Inset");
          ...
          case ADVANCED_LA:
            if (draw) {
              Draw_Menu_Item(row, Icon::MaxAccelerated, "Lin Advance Kp");
          ...
          case ADVANCED_LOAD:
            if (draw) {
              Draw_Menu_Item(row, Icon::WriteEEPROM, "Load Length");
          ...
          case ADVANCED_UNLOAD:
            if (draw) {
              Draw_Menu_Item(row, Icon::ReadEEPROM, "Unload Length");
          ...
          case ADVANCED_COLD_EXTRUDE:
            if (draw) {
              Draw_Menu_Item(row, Icon::Cool, "Min Extrusion T");
          ...
          case ADVANCED_FILSENSORENABLED:
            if (draw) {
              Draw_Menu_Item(row, Icon::Extruder, "Filament Sensor");
            ...
            case ADVANCED_FILSENSORDISTANCE:
              if (draw) {
                Draw_Menu_Item(row, Icon::MaxAccE, "Runout Distance");
          ...
          case ADVANCED_POWER_LOSS:
            if (draw) {
              Draw_Menu_Item(row, Icon::Motion, "Power-loss recovery");
      ...
      case ProbeMenu:
        ...
        switch (item) {
          case PROBE_BACK:
            ...
            case PROBE_XOFFSET:
              if (draw) {
                Draw_Menu_Item(row, Icon::StepX, "Probe X Offset");
            ...
            case PROBE_YOFFSET:
              if (draw) {
                Draw_Menu_Item(row, Icon::StepY, "Probe Y Offset");
            ...
            case PROBE_TEST:
              if (draw) {
                Draw_Menu_Item(row, Icon::StepY, "M48 Probe Test");
              }
              else {
            ...
            case PROBE_TEST_COUNT:
              if (draw) {
                Draw_Menu_Item(row, Icon::StepY, "Probe Test Count");
    ...
    case Info:
        ...
        case INFO_BACK:
          if (draw) {
            Draw_Menu_Item(row, Icon::Back, "Back");
              ...
              Draw_Menu_Item(INFO_PRINTCOUNT, Icon::HotendTemp, row1, row2, false, true);
              ...
              Draw_Menu_Item(INFO_PRINTTIME, Icon::PrintTime, row1, row2, false, true);
            #endif
            
            Draw_Menu_Item(INFO_SIZE, Icon::PrintSize, MACHINE_SIZE, NULL, false, true);
            Draw_Menu_Item(INFO_VERSION, Icon::Version, SHORT_BUILD_VERSION, "Build Number: v" BUILD_NUMBER, false, true);
            Draw_Menu_Item(INFO_CONTACT, Icon::Contact, CORP_WEBSITE_E, NULL, false, true);
          }
          else {
            if (menu == Info)
      ...
      case Leveling:
        ...
        switch (item) {
          ...
          case LEVELING_ACTIVE:
            if (draw) {
              Draw_Menu_Item(row, Icon::StockConfiguraton, "Leveling Active");
            ...
            case LEVELING_GET_TILT:
              if (draw) {
                Draw_Menu_Item(row, Icon::Tilt, "Autotilt Current Mesh");
              }
              else {
          ...
          case LEVELING_GET_MESH:
            if (draw) {
              Draw_Menu_Item(row, Icon::Mesh, "Create New Mesh");
            }
            else {
              Popup_Handler(Home);
              gcode.home_all_axes(true);
          ...
          case LEVELING_MANUAL:
            if (draw) {
              Draw_Menu_Item(row, Icon::Mesh, "Manual Tuning", NULL, true);
            }
            else {
          ...
          case LEVELING_VIEW:
            if (draw) {
              Draw_Menu_Item(row, Icon::Mesh, "Mesh Viewer", NULL, true);
            }
            else {
              #if ENABLED(AUTO_BED_LEVELING_UBL)
                if (ubl.storage_slot < 0) {
          ...
          case LEVELING_SETTINGS:
            if (draw) {
              Draw_Menu_Item(row, Icon::Step, "Leveling Settings", NULL, true);
            }
            else {
          ...
          case LEVELING_LOAD:
            if (draw) {
              Draw_Menu_Item(row, Icon::ReadEEPROM, "Load Mesh");
            }
            else {
              if (ubl.storage_slot < 0) {
                Popup_Handler(MeshSlot);
          ...
          case LEVELING_SAVE:
            if(draw) {
              Draw_Menu_Item(row, Icon::WriteEEPROM, "Save Mesh");
            }
            else {
              if (ubl.storage_slot < 0) {
                Popup_Handler(MeshSlot);
      ...
      case LevelView:
        ...
        switch (item) {
          ...
          case LEVELING_VIEW_MESH:
            if (draw) {
              Draw_Menu_Item(row, Icon::PrintSize, "Mesh Viewer", NULL, true);
            }
            else {
          ...
          case LEVELING_VIEW_TEXT:
            if (draw) {
              Draw_Menu_Item(row, Icon::Contact, "Viewer Show Values");
          ...
          case LEVELING_VIEW_ASYMMETRIC:
            if (draw) {
              Draw_Menu_Item(row, Icon::Axis, "Viewer Asymmetric");
      ...
      case LevelSettings:
        ...
        switch (item) {
          ...
          case LEVELING_SETTINGS_FADE:
              if (draw) {
                Draw_Menu_Item(row, Icon::Fade, "Fade Mesh within");
            ...
            case LEVELING_SETTINGS_TILT:
                if (draw) {
                  Draw_Menu_Item(row, Icon::Tilt, "Tilting Grid Size");
            ...
            case LEVELING_SETTINGS_PLANE:
                if (draw) {
                  Draw_Menu_Item(row, Icon::ResumeEEPROM, "Convert Mesh to Plane");
                }
                else {
            ...
            case LEVELING_SETTINGS_ZERO:
                if (draw) {
                  Draw_Menu_Item(row, Icon::Mesh, "Zero Current Mesh");
                }
                else {
              ...
              case LEVELING_SETTINGS_UNDEF:
                if (draw) {
                  Draw_Menu_Item(row, Icon::Mesh, "Clear Current Mesh");
                }
                else {
                  ubl.invalidate();
                }
      ...
      case LevelManual:
        ...
        switch (item) {
          ...
          case LEVELING_M_X:
            if (draw) {
              Draw_Menu_Item(row, Icon::MoveX, "Mesh Point X");
          ...
          case LEVELING_M_Y:
            if (draw) {
              Draw_Menu_Item(row, Icon::MoveY, "Mesh Point Y");
          ...
          case LEVELING_M_NEXT:
            if (draw) {
              Draw_Menu_Item(row, Icon::More, "Next Point");
            }
            else {
          ...
          case LEVELING_M_OFFSET:
            if (draw) {
              Draw_Menu_Item(row, Icon::SetZOffset, "Point Z Offset");
          ...
          case LEVELING_M_GOTO_VALUE: 
            if (draw) {
              Draw_Menu_Item(row, Icon::StockConfiguraton, "Go to Mesh Z Value");
          ...
          case LEVELING_M_UNDEF:
            if (draw) {
              Draw_Menu_Item(row, Icon::ResumeEEPROM, "Clear Point Value");
            }
            else {
      ...
      case UBLMesh:
        ...
        switch (item) {
          ...
          case UBL_M_NEXT:
            if (draw) {
              if (mesh_conf.mesh_x != (GRID_MAX_POINTS_X-1) || mesh_conf.mesh_y != (GRID_MAX_POINTS_Y-1))
                ...
                Draw_Menu_Item(row, Icon::More, "Save Mesh");
            }
            else {
              if (mesh_conf.mesh_x != (GRID_MAX_POINTS_X-1) || mesh_conf.mesh_y != (GRID_MAX_POINTS_Y-1)) {
                if ((mesh_conf.mesh_x == (GRID_MAX_POINTS_X-1) && mesh_conf.mesh_y % 2 == 0) || (mesh_conf.mesh_x == 0 && mesh_conf.mesh_y % 2 == 1)) {
          ...
          case UBL_M_PREV:
            if (draw) {
              Draw_Menu_Item(row, Icon::More, "Previous Point");
            }
            else {
          ...
          case UBL_M_DOWN:
            if (draw) {
              Draw_Menu_Item(row, Icon::Axis, "Microstep Down");
            }
            else {
              if (mesh_conf.mesh_z_values[mesh_conf.mesh_x][mesh_conf.mesh_y] > MIN_Z_OFFSET) {
                mesh_conf.mesh_z_values[mesh_conf.mesh_x][mesh_conf.mesh_y] -= 0.01;
      ...
      case ManualMesh:
        ...
        switch (item) {
          case MMESH_BACK:
            if (draw) {
              Draw_Menu_Item(row, Icon::Back, "Cancel");
            }
            else {
          ...
          case MMESH_OLD:
            ...
            if (draw) {
              Draw_Menu_Item(row, Icon::Zoffset, "Goto Mesh Value");
    ...
    case Tune:
      ...
      switch (item) {
        ...
        case TUNE_SPEED:
          if (draw) {
            Draw_Menu_Item(row, Icon::Speed, "Print Speed");
          ...
          case TUNE_ZOFFSET:
            if (draw) {
              Draw_Menu_Item(row, Icon::FanSpeed, "Z-Offset");
              Draw_Float(zoffsetvalue, row, false, 100);
            }
            else {
              Modify_Value(zoffsetvalue, MIN_Z_OFFSET, MAX_Z_OFFSET, 100);
          ...
          case TUNE_ZUP:
            if (draw) {
              Draw_Menu_Item(row, Icon::Axis, "Z-Offset Up");
            }
            else {
              if (zoffsetvalue < MAX_Z_OFFSET) {
                gcode.process_subcommands_now_P(PSTR("M290 Z0.01"));
          ...
          case TUNE_ZDOWN:
            if (draw) {
              Draw_Menu_Item(row, Icon::AxisD, "Z-Offset Down");
            }
            else {
              if (zoffsetvalue > MIN_Z_OFFSET) {
                gcode.process_subcommands_now_P(PSTR("M290 Z-0.01"));
    ...
    case PreheatHotend:
        ...
        switch (item) {
          ...
          case PREHEATHOTEND_CONTINUE:
            if (draw) {
              Draw_Menu_Item(row, Icon::SetEndTemp, "Continue");
            }
            else {
              Popup_Handler(Heating);
              thermalManager.wait_for_hotend(0);
          ...
          case PREHEATHOTEND_CUSTOM:
            if (draw) {
              Draw_Menu_Item(row, Icon::Temperature, "Custom");
              Draw_Float(thermalManager.temp_hotend[0].target, row, false, 1);
            }
            else {
...
void CrealityDWINClass::Popup_Handler(PopupID popupid, bool option/*=false*/) {
  ...
  switch (popupid) {
    ...
    case ManualProbing:
      ...
      break;
    case Level:
      Draw_Popup("Auto Bed Leveling", "Please wait until done.", "", Wait, Icon::AutoLeveling);
      break;
    case Home:
      Draw_Popup(option ? "Parking" : "Homing", "Please wait until done.", "", Wait, Icon::BLTouch);
      break;
    case MoveWait:
      Draw_Popup("Moving to Point", "Please wait until done.", "", Wait, Icon::BLTouch);
      break;
    case Heating:
      Draw_Popup("Heating", "Please wait until done.", "", Wait, Icon::BLTouch);
      break;
    case FilLoad:
      Draw_Popup(option ? "Unloading Filament" : "Loading Filament", "Please wait until done.", "", Wait, Icon::BLTouch);
      break;
    case FilChange:
      Draw_Popup("Filament Change", "Please wait for prompt.", "", Wait, Icon::BLTouch);
      break;
    case TempWarn:
      Draw_Popup(option ? "Nozzle temp too low!" : "Nozzle temp too high!", "", "", Wait, option ? Icon::TempTooLow : Icon::TempTooHigh);
      break;
    case Runout:
      Draw_Popup("Filament Runout", "", "", Wait, Icon::BLTouch);
      break;
    case PIDWait:
      Draw_Popup("PID Autotune", "in process", "Please wait until done.", Wait, Icon::BLTouch);
      break;
    case Resuming:
      Draw_Popup("Resuming Print", "Please wait until done.", "", Wait, Icon::BLTouch);
      break;
    default:
      break;
  }
...
void CrealityDWINClass::Menu_Control() {
  ...
  if (encoder_diffState == ENCODER_DIFF_CW && selection < Get_Menu_Size(active_menu)) {
    ...
    if (selection > scrollpos+MROWS) {
      scrollpos++;
      DWIN_Frame_AreaMove(1, 2, MLINE, Color_Bg_Black, 0, 31, geometry.screen.w, 349);
    ...
    if (selection < scrollpos) {
      scrollpos--;
      DWIN_Frame_AreaMove(1, 3, MLINE, Color_Bg_Black, 0, 31, geometry.screen.w, 349);
      Menu_Item_Handler(active_menu, selection);
    }
    DWIN_Draw_Rectangle(1, GetColor(eeprom_settings.cursor_color, Rectangle_Color), 0, MBASE(selection-scrollpos) - 18, 14, MBASE(selection-scrollpos) + 33);
  }
  ...
  if (encoder_diffState == ENCODER_DIFF_NO) {
    if (selection > 0) {
      ...
      if (pos > MENU_CHAR_LIMIT) {
        ...
        Draw_Menu_Item(selection-scrollpos, card.flag.filenameIsDir ? Icon::More : Icon::File, name);
        if (-pos >= MENU_CHAR_LIMIT)
          filescrl = 0;
        filescrl++;
        DWIN_UpdateLCD();
