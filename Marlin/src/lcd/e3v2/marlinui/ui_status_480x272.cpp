...
FORCE_INLINE void _draw_axis_value(const AxisEnum axis, const char *value, const bool blink, const uint16_t x, const uint16_t y) {
  ...
  if (!ui.did_first_redraw) {
    ...
    DWIN_Draw_String(true, Font::f16x32, Color_IconBlue, Color_Bg_Black, x + (vallen * 14 - 14) / 2, y + 2, S(dwin_string.string()));
  }
  ...
  DWIN_Draw_String(true, Font::f14x28, StdColor::White, Color_Bg_Black, x, y + 32, S(dwin_string.string()));
}
  ...
  FORCE_INLINE void _draw_e_value(const_float_t value, const uint16_t x, const uint16_t y) {
    ...
    if (!ui.did_first_redraw) {
      ...
      dwin_string.set("E         ");
      DWIN_Draw_String(true, Font::f16x32, Color_IconBlue, Color_Bg_Black, x + (4 * 14 / 2) - 7, y + 2, S(dwin_string.string()));
    }
    ...
    DWIN_Draw_String(true, Font::f14x28, Color_IconBlue, Color_Bg_Black, x + (5 * 14), y + 32, S(scale == 1 ? "mm      " : "cm      "));
  }
...
FORCE_INLINE void _draw_fan_status(const uint16_t x, const uint16_t y) {
  ...
  if (fan_on) {
    ...
    dwin_string.add('%');
    DWIN_Draw_String(true, Font::f14x28, StdColor::White, Color_Bg_Black, x, y + STATUS_FAN_HEIGHT, S(dwin_string.string()));
  }
  else {
...
FORCE_INLINE void _draw_heater_status(const heater_id_t heater, const uint16_t x, const uint16_t y) {
  ...
  if (!ui.did_first_redraw || t_draw) {
    ...
    DWIN_Draw_String(true, Font::f14x28, StdColor::White, Color_Bg_Black, x, y, S(dwin_string.string()));
  }
  ...
  if (!ui.did_first_redraw || c_draw) {
    ...
    dwin_string.add(LCD_STR_DEGREE);
    DWIN_Draw_String(true, Font::f14x28, StdColor::White, Color_Bg_Black, x, y + 70, S(dwin_string.string()));
  }
}
...
FORCE_INLINE void _draw_feedrate_status(const char *value, uint16_t x, uint16_t y) {
  if (!ui.did_first_redraw) {
    ...
    DWIN_Draw_String(true, Font::f14x28, Color_IconBlue, Color_Bg_Black, x, y, S(dwin_string.string()));
  }

  dwin_string.set(value);
  dwin_string.add(PSTR("%"));
  DWIN_Draw_String(true, Font::f14x28, StdColor::White, Color_Bg_Black, x + 14, y, S(dwin_string.string()));
}
...
void MarlinUI::draw_status_screen() {
  ...
  duration_t time;
    ...
    DWIN_Draw_String(true, Font::f14x28, StdColor::White, Color_Bg_Black, (LCD_PIXEL_WIDTH - ((dwin_string.length() + 1) * 14)), 290, S(dwin_string.string()));
    ...
    dwin_string.set(" ");
    dwin_string.add(buffer);
    DWIN_Draw_String(true, Font::f14x28, StdColor::White, Color_Bg_Black, 280, 100, S(dwin_string.string()));
      ...
      if (show_e_total && TERN1(SHOW_REMAINING_TIME, !blink)) { // if SHOW_REMAINING_TIME is also
        ...
        DWIN_Draw_String(true, Font::f14x28, Color_IconBlue, Color_Bg_Black, 249, 135, S("E"));
        dwin_string.set(ui16tostr5rj(e_move_accumulator * escale));
        DWIN_Draw_String(true, Font::f14x28, StdColor::White, Color_Bg_Black, 263, 135, S(dwin_string.string()));
        DWIN_Draw_String(true, Font::f14x28, Color_IconBlue, Color_Bg_Black, 333, 135, S(escale==1 ? "mm" : "cm"));
      }
    #endif
    #if ENABLED(SHOW_REMAINING_TIME)
      if (!show_e_total || blink) {
        DWIN_Draw_String(true, Font::f14x28, Color_IconBlue, Color_Bg_Black, 249, 135, S(" R "));
        time = get_remaining_time();
        time.toDigital(buffer);
        dwin_string.set(buffer);
        DWIN_Draw_String(true, Font::f14x28, StdColor::White, Color_Bg_Black, 291, 135, S(dwin_string.string()));
      }
    #endif
  #endif
  ...
  if (p_draw) {
    ...
    //if (pb_solid < old_solid)
      ...
      dwin_string.add(PSTR("%"));
      DWIN_Draw_String(
        false, Font::f16x32, Percent_Color, Color_Bg_Black,
